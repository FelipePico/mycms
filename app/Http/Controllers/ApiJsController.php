<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Config, Auth;
use App\Http\Models\Product, App\Http\Models\Favorite;


class ApiJsController extends Controller
{
    public function __construct(){
        $this->middleware('auth')->except(['getProductsSection']);
    }

    function getProductsSection($section, Request $request){
        $items_x_page = Config::get('mycms.products_per_page');
        switch ($section):
            case 'home':
                $products = Product::where('status', 1)->paginate($items_x_page);
                break;
            
            default:
                $products = Product::where('status', 1)->paginate($items_x_page);//inRandomOrder()-> ESTO VA AQUÍ Y EN EL DE ARRIBA
                break;
        endswitch;

        return $products;
    }

    function postFavoriteAdd($object, $module, Request $request){
        $query = Favorite::where('user_id', Auth::id())->where('module', $module)->where('object_id', $object)->count();
        if($query > 0):
            $data = [ 'status'=> 'error', 'msg' => 'este item ya está en tus favoritos.' ];
        else:
            $favorite = new Favorite;
            $favorite->user_id = Auth::id();
            $favorite->module = $module;
            $favorite->object_id = $object;
            if($favorite->save()):
                $data = [ 'status'=> 'succes', 'msg' => 'se guardó en favoritos' ];
            endif;
        endif;

        return response()->json($data);
        //

    }
}
